...
class BookingController extends Controller
{
    ...
    public function getUserBookings(Request $request)
    {
        ...
        $bookings = DB::table('bookings')
            ->join('vehicles', 'bookings.id_vehicle', '=', 'vehicles.id')
            ->leftJoin('payments', 'bookings.id', '=', 'payments.id_booking') // join ke payments
            ->where('bookings.id_user', $user->id)
            ->when($status && $status !== 'booking', function ($query) use ($status) {
                return $query->where('bookings.booking_status', $status);
            })
            ->when($status === 'booking', function ($query) {
                return $query->whereIn('bookings.booking_status', ['ongoing', 'completed', 'cancelled']);
            })
            ->select(
                'bookings.*',
                'vehicles.vehicle_name',
        ...
        $result = $bookings->map(function ($item) {
            ...
            return [
                ...
                // Tambahan kolom dari booking
                'start_date' => $item->start_date,
                'end_date' => $item->end_date,
                'pickup_location' => $item->pickup_location,

                // Tambahan dari payment
